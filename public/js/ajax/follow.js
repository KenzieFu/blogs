  ...
  $('.follow-btn').click(function(){
    ...
    if ($button.hasClass('following-btn')) {
      $.ajax({
        type:'POST',
        url:'/ajaxUnfollow',
        data:{unfollow:followID, profile:profile},
...
/*       $.post('http://localhost/sosmed/core/ajax/follow.php', {unfollow:followID, profile:profile}, function(data){
      ...
      $.ajax({
        type:'POST',
        url:'/ajaxFollow',
        data:{follow:followID, profile:profile},
        success:function(data)
        {
 ...
 /*      $.post('http://localhost/sosmed/core/ajax/follow.php', {follow:followID, profile:profile}, function(data){
        data = JSON.parse(data);
        $button.removeClass('follow-btn');
        $button.addClass('following-btn');
        $button.text('Following');
